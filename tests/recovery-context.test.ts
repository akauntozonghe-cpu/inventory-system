...
it("does not recommend cancelling stocktakes for a notification or communication failure",()=>{for(const [route,code] of [["/notifications","PUSH_FAILED"],["/stocktake/s","NETWORK_ERROR"],["/marketplace","HTTP_503"]]){const codes=recoveryCheckCodes(route,code);expect(codes).toEqual(code==="PUSH_FAILED"?["CHECK_DATABASE_CONNECTION","CHECK_PUSH_CONFIGURATION","CHECK_DEVICE_NOTIFICATION"]:["CHECK_DATABASE_CONNECTION"]);expect(recoveryActionAllowed("CANCEL_SESSION",codes)).toBe(false);expect(recoveryActionAllowed("SYNC_PRODUCT_METADATA",codes)).toBe(false);}});

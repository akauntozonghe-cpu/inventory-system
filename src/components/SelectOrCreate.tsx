...
export default function SelectOrCreate({ value, options, onChange, label, required = false }: {
  value: string; options: string[]; onChange: (value: string) => void; label: string; required?: boolean;
}) {
  const id = useId();
  ...
  return <div className="mt-2 space-y-2">
    <select aria-label={`${label}を選択`} value={custom ? "__CREATE__" : value} required={required && !custom}

...
export default function PageNavigation() {
  ...
  useEffect(()=>{
    dirty.current=false;setDestination(null);setBrowserBack(false);
    ...
    const input=(event:Event)=>{const element=event.target as HTMLInputElement;if(element.closest("form")&&!["search","checkbox","radio"].includes(element.type))dirty.current=true;};
    const draft=(event:Event)=>{dirty.current=Boolean((event as CustomEvent).detail?.dirty);};
    const click=(event:MouseEvent)=>{
      ...
      if(dirty.current){event.preventDefault();event.stopPropagation();setDestination(link.pathname+link.search);}
    };
  ...
  const move=async(href:string)=>{setBusy(true);await leaveCurrentStocktakes();dirty.current=false;setBrowserBack(false);setDestination(null);router.push(href);setBusy(false);};
  if(publicPage(pathname))return null;
  return <>{entryNotice&&<p role="status" className="bg-blue-50 px-4 py-2 text-sm">作業はホームのメニューから開始してください。</p>}<nav aria-label="画面の移動" className="print:hidden flex flex-wrap items-center justify-between gap-2 border-b bg-white px-4 py-2 text-sm">{pathname!=="/"?<Link className="rounded-lg px-2 py-2 font-bold text-blue-800" href={parent.href}>← {parent.label}</Link>:<span className="px-2 font-bold">ホーム</span>}<Link className="rounded-lg px-3 py-2 font-bold text-blue-800" href="/notifications">通知を確認</Link></nav>{(destination||browserBack)&&<div className="fixed inset-0 z-[550] grid place-items-center bg-slate-950/60 p-4"><section role="dialog" aria-modal="true" aria-labelledby="navigation-title" className="w-full max-w-md rounded-2xl bg-white p-5"><h2 id="navigation-title" className="text-xl font-black">{destination?"入力内容を確認してください":"画面内のボタンで移動してください"}</h2><p className="my-3">{destination?"保存していない入力があります。移動すると、この入力は引き継がれません。":"作業の順序を保つため、ブラウザの戻る・進むでは移動しません。画面内の戻る・次の操作を使ってください。"}</p><div className="flex flex-wrap gap-2"><button disabled={busy} className="rounded-xl bg-blue-700 p-3 font-bold text-white" onClick={()=>{setDestination(null);setBrowserBack(false);}}>この画面で続ける</button><button disabled={busy} className="rounded-xl border p-3 font-bold" onClick={()=>{if(!destination&&dirty.current){setDestination(parent.href);setBrowserBack(false);}else void move(destination??parent.href);}}>{destination?"保存せずに移動":parent.label}</button></div></section></div>}</>;
}

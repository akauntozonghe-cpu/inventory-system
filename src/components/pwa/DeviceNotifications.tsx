...
export default function DeviceNotifications() {
  ...
  const act = async (action: "SETUP" | "ENABLE" | "DISABLE" | "TEST" | "LOCAL" | "RECONNECT") => {
    ...
    try {
      if (action === "SETUP") {
        ...
        setMessage(await saveDevicePush(subscription, action === "TEST" ? "TEST" : "SUBSCRIBE"));localStorage.setItem(PUSH_ENABLED,"1");setEnabled(true);await load();
      }

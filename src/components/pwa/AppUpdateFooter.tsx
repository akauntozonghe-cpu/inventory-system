...
export default function AppUpdateFooter() {
  ...
  useEffect(()=>{const check=()=>{const element=document.activeElement;const input=element instanceof HTMLInputElement||element instanceof HTMLTextAreaElement;setKeyboardOpen(Boolean(input&&window.visualViewport&&window.innerHeight-window.visualViewport.height>150));};document.addEventListener("focusin",check);document.addEventListener("focusout",check);window.visualViewport?.addEventListener("resize",check);return()=>{document.removeEventListener("focusin",check);document.removeEventListener("focusout",check);window.visualViewport?.removeEventListener("resize",check);};},[]);
  ...
  return <footer ref={panel} className={`${keyboardOpen?"hidden":""} fixed inset-x-0 bottom-0 z-[65] print:hidden border-t bg-white px-3 py-0 text-xs text-slate-600`}>
    <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-2"><span className="flex items-center gap-2"><span aria-hidden="true" className={`h-2 w-2 rounded-full ${online?"bg-emerald-500":"bg-amber-500"}`}/>{online?"オンライン":"オフライン：通信復帰後に同期"}</span><NotificationPermissionLink/></div>

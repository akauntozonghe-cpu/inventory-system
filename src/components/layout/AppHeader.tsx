...
export default function AppHeader({parent}:{parent:{href:string;label:string}}) {
  ...
  const [open,setOpen]=useState(false),[unread,setUnread]=useState(0),[error,setError]=useState(""),[busy,setBusy]=useState(false);
  const {user,refresh}=useAppAccess();
  ...
  useEffect(()=>{const refresh=()=>{void load().catch(()=>{});};window.addEventListener("inventory:notifications-read",refresh);return()=>window.removeEventListener("inventory:notifications-read",refresh);},[]);
  ...
  return <header className="sticky top-0 z-[60] border-b border-slate-200 bg-white/95 text-slate-950 shadow-sm backdrop-blur print:hidden">
    ...
    <dialog ref={drawer} id="app-menu" aria-labelledby="app-menu-title" onCancel={event=>{event.preventDefault();setOpen(false);}} onClick={event=>{if(event.target===event.currentTarget)setOpen(false);}} className="fixed inset-y-0 left-auto right-0 m-0 h-dvh max-h-none w-[min(90vw,360px)] max-w-none overflow-y-auto border-l bg-white p-0 text-slate-950 shadow-2xl backdrop:bg-slate-950/50">
      ...
      <div className="px-4 pt-3"><button className="w-full rounded-xl border p-3 text-left text-sm font-bold" onClick={()=>{setOpen(false);adminMode.open();}}>{adminMode.isAdmin?"管理者の操作":adminMode.active?"一時管理者の操作・認証の終了":"管理者認証"}</button></div><nav aria-label="共通メニュー" className="space-y-4 p-4">{user&&visibleAppMenu(user).map(group=><section key={group.title}><h3 className="mb-1 px-3 text-xs font-bold text-slate-500">{group.title}</h3>{group.links.map(link=><Link key={link.href} href={link.href} aria-current={pathname===link.href?"page":undefined} onClick={()=>setOpen(false)} className={`block rounded-xl px-3 py-3 text-sm font-bold ${pathname===link.href?"bg-teal-50 text-teal-900":"hover:bg-slate-100"}`}>{link.label}</Link>)}</section>)}</nav>
      <div className="space-y-2 border-t p-4">{(error||stale)&&<div role="alert" className="text-sm text-red-700">{error||"MENU_SYNC_FAILED：最新情報を確認できませんでした。"}<button className="ml-2 underline" onClick={()=>void Promise.all([refresh(),load()]).catch(()=>setError("MENU_LOAD_FAILED：メニューを読み込めませんでした。"))}>再読み込み</button></div>}<button className="flex w-full items-center gap-3 rounded-xl border p-3 font-bold" onClick={()=>{setOpen(false);window.dispatchEvent(new Event("inventory:app-update"));}}><RefreshCw size={18}/>アプリの更新を確認</button><Link href="/notifications#device-permissions" onClick={()=>setOpen(false)} className="block rounded-xl border p-3 font-bold">端末の通知・使用許可</Link><button disabled={busy} className="flex w-full items-center gap-3 rounded-xl p-3 font-bold text-slate-600" onClick={()=>void logout().catch(()=>setError("LOGOUT_FAILED：ログアウトできませんでした。"))}><LogOut size={18}/>{busy?"ログアウト中…":"ログアウト"}</button></div>
    </dialog>
  </header>;
}

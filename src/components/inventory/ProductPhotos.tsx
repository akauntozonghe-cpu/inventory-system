...
export default function ProductPhotos({itemId,canEdit=false}:{itemId:string;canEdit?:boolean}){
  ...
  return <section className="my-2 rounded-xl border bg-white p-3">
    <div className="flex items-center gap-3">{photos[0]?<button type="button" onClick={()=>setExpanded(true)} aria-label="商品の写真を開く" className="shrink-0"><Image unoptimized width={96} height={72} src={base+"/"+photos[0].id+"?thumbnail=1"} alt="商品写真 1" className="h-18 w-24 rounded-lg object-contain"/></button>:<span className="text-xs text-slate-400">写真なし</span>}<button type="button" onClick={()=>setExpanded(true)} className="min-h-11 rounded-lg border px-3 text-sm font-bold">{photos.length?"写真を見る（"+photos.length+"枚）":canEdit?"写真を追加":"写真を見る"}</button></div>

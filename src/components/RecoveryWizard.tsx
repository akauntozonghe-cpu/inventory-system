...
export default function RecoveryWizard({contextRoute:sourceRoute,errorCode,initialReportId,onReturnToWork}:{contextRoute?:string;errorCode?:string;initialReportId?:string;onReturnToWork?:()=>void}) {
  ...
  return <section className="mb-6 rounded-2xl border-2 border-blue-300 bg-white p-5 text-slate-950" aria-label="復旧ウィザード">
    ...
    {review&&<div className="fixed inset-0 z-[400] grid place-items-center bg-slate-950/60 p-4"><section role="dialog" aria-modal="true" aria-label="復旧処置の確認" className="w-full max-w-lg rounded-2xl bg-white p-5"><h3 className="text-xl font-black">{review.label}</h3><p className="my-3">{review.action==="CANCEL_SESSION"?"棚卸を取り消します。入力と対象の記録を確認してください。在庫の数量はこの操作では変更しません。":review.action==="SYNC_PRODUCT_METADATA"?"商品マスターに合わせて在庫の分類・メーカー情報と分類の選択肢を修復します。数量は変更しません。":"選択した対象だけに処置を実行し、その後に再診断します。"}</p><textarea aria-label="復旧の理由" value={reason} onChange={e=>setReason(e.target.value)} placeholder="処置の理由（必須）" className="w-full rounded-xl border p-3"/><div className="mt-3 flex gap-3"><button disabled={busy||!reason.trim()} className="rounded-xl bg-blue-700 p-3 font-bold text-white disabled:opacity-40" onClick={()=>{const action={...review,reason};setReview(null);void execute(action);}}>この内容で実行</button><button className="rounded-xl border p-3" onClick={()=>setReview(null)}>戻る</button></div></section></div>}

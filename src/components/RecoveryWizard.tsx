"use client";
import { recoveryCheckCodes, recoverySessionId, recoveryNextStep } from "@/lib/recovery-context";
...
export default function RecoveryWizard({contextRoute,errorCode}:{contextRoute?:string;errorCode?:string}) {
  const [reports,setReports]=useState<Array<{id:string;code:string;message:string;recoveryAttempts:number;sessionId?:string|null}>>([]);
  ...
  const affectedSessionId=recoverySessionId(contextRoute,selectedReport?.sessionId);
  ...
  useEffect(()=>{if(contextRoute){void readReports().catch(()=>setError("RECOVERY_REPORTS_FAILED：復旧対象を取得できませんでした。"));return;}let active=true;const version=diagnosisVersion.current;void fetchFresh("/api/admin/system-check").then(read).then(value=>{if(active&&version===diagnosisVersion.current&&value.runs?.[0]){setRun(value.runs[0]);setStep(1);setHistoryLoaded(true);}}).catch(()=>{});return()=>{active=false;};},[contextRoute,readReports]);
  ...
  async function diagnose(recheck = false) {
    ...
    try {
      const value = await read(await fetchFresh("/api/admin/system-check", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "RUN_AUTO", contextRoute, reportId:selectedReport?.id, errorCode }) }));
  ...
  async function loadTargets() {
    ...
    try {
      ...
      setSessions((value.activeSessions ?? []).filter((session:Session)=>!contextRoute||session.id===affectedSessionId));
  ...
  async function execute(action: Action) {
    ...
    try {
      const value = await read(await fetchFresh("/api/admin/system-check/remediate", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({...action,contextRoute,reportId:selectedReport?.id}) }));
  ...
  return <section className="mb-6 rounded-2xl border-2 border-blue-300 bg-white p-5 text-slate-950" aria-label="復旧ウィザード">
    <h2 className="text-2xl font-black">診断から復旧まで</h2>{contextRoute&&<div className="my-3"><label className="font-bold">復旧するエラー<select aria-label="復旧するエラー" disabled={busy} className="mt-2 w-full rounded-xl border p-3" value={selectedReportId} onChange={event=>{setSelectedReportId(event.target.value);setRun(null);setStep(0);setRechecked(false);setTargets([]);setSessions([]);setVerified(false);setResult("");}}><option value="">現在の画面を診断</option>{reports.map(report=><option key={report.id} value={report.id}>{report.code}：{report.message}</option>)}</select></label><p className="mt-2 text-sm">選んだエラーに関係する点検と処置だけを表示します。</p></div>}<p className="mt-2 text-sm">1件ずつ、対象と影響を確認して処置し、再診断で結果を確かめます。</p>{historyLoaded&&<p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm">前回の点検結果を表示しています。現在の状態は「再チェック」で確認してください。</p>}
    ...
    <div className="space-y-3">{issues.map((check) => <article key={check.code} className="rounded-xl border p-4">
      ...
      </> : <><p className="my-2 text-sm">{advice[check.code]?.text ?? "該当する設定を確認し、修正後に再チェックしてください。"}</p><Link href={advice[check.code]?.href ?? "/admin/error-reports"} className="font-bold text-blue-700 underline">対応画面へ進む</Link></>}
    </article>)}</div>
    {run&&(!contextRoute||Boolean(affectedSessionId&&relevant?.includes("CHECK_REVIEW_RECORDS")))&&<section className="my-4 rounded-xl border p-4"><h3 className="font-black">対象の棚卸を確認する</h3><p className="my-2 text-sm">確認待ち・完了済みは棚卸管理で再開します。取消は入力内容を確認してから実行します。</p><button disabled={busy} className="rounded-lg border px-3 py-2 font-bold" onClick={()=>void loadTargets()}>対象の棚卸を確認</button>{!contextRoute&&<Link href="/admin/stocktake" className="ml-3 font-bold text-blue-700 underline">確認待ち・完了済みを再開</Link>}{sessions.map(session=><div key={session.id} className="mt-3 rounded-xl bg-slate-50 p-3"><p className="font-bold">{session.title} ／ {session.scopeLabel}</p><Link href={"/stocktake/"+session.id} className="mr-3 text-blue-700 underline">記録を確認</Link>{session.status==="PAUSED"&&<button disabled={busy} className="rounded-lg border p-2" onClick={()=>{setReason("");setReview({action:"RESUME_SESSION",sessionId:session.id,label:session.title+"を再開"});}}>再開する</button>}{session.status==="IN_PROGRESS"&&<button disabled={busy} className="rounded-lg border p-2" onClick={()=>{setReason("");setReview({action:"PAUSE_SESSION",sessionId:session.id,label:session.title+"を中断"});}}>中断する</button>}<button disabled={busy} className="ml-2 rounded-lg border p-2 text-red-700" onClick={()=>{setReason("");setReview({action:"CANCEL_SESSION",sessionId:session.id,label:session.title+"を取消"});}}>取消を確認</button></div>)}</section>}
    <div className="mt-4 flex flex-wrap gap-3"><button disabled={busy} onClick={() => void diagnose(Boolean(run))} className="rounded-xl bg-slate-900 px-5 py-3 font-black text-white disabled:opacity-50">{busy ? `${steps[step]}中…` : run ? "次へ：再チェックする" : "次へ：このエラーを診断"}</button>{rechecked && !error && <Link href={nextStep.href} className="rounded-xl border px-5 py-3 font-bold">{nextStep.text}</Link>}</div>

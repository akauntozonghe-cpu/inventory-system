...
export default function LabelPrintDialog({ labels, onClose, onComplete, onRefresh, canEdit = false }: {
    ...
    async function preview() {
        ...
        try {
            ...
            if (qr) {
                ...
                html = barcodePrintDocument(targets.flatMap(row => Array.from({ length: repeat }, () => ({ name: row.name, barcode: row.barcode! }))), layout, scale, includeName);
    ...
    {qr ? <p className="my-3 rounded-xl bg-blue-50 p-3 text-sm">大分類QRは箱に貼って分類で絞り込み、保管場所QRは棚に貼ってその場所の在庫を表示します。読み取るだけでは在庫の移動・数量変更は行いません。QRは50mm角、ラベルは60×70mmです。</p> : <BarcodePrintOptions scale={scale} onScale={setScale} includeName={includeName} onIncludeName={setIncludeName}/>}

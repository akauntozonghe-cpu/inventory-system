...
export default function FieldScanButton({ kind, onRead, disabled = false }: {
    ...
    const notice = kind === "JAN"
        ? "商品に付いたJAN、または登録済みQRを枠の中に入れてください。読取後に内容を確認して保存します。"
        : isScope
            ? "大分類・小分類・保管場所のQRを読み取って棚卸範囲を選びます。開始は内容を確認してから行います。"
            : "読み取った内容を入力します。保存・棚卸開始は、内容を確認してから行ってください。";
    return <><button type="button" disabled={disabled} aria-label={label + "をカメラで読み取る"} title={label + "を読み取って入力"} onClick={() => setOpen(true)} className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-slate-300 bg-white text-indigo-700 disabled:opacity-40"><Camera size={22}/></button>{open && <UnifiedScanner title={kind === "JAN" ? "JAN・QRを読み取る" : label + "を読み取って入力"} notice={notice} onProduct={code => { if (kind !== "JAN")
        ...
        return unsupported("大分類QR"); onRead({ kind: "MAJOR", value: name }); }} onMinorCategory={(name, parentName) => { if (kind !== "MINOR" && kind !== "SCOPE")

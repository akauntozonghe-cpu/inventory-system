...
const LabelPrintDialog = dynamic(() => import("./LabelPrintDialog"), { ssr: false });
...
export default function LabelPrintPanel({ data, onRefresh }: {
    ...
    return <section id="label-print" className="mt-6 scroll-mt-24 rounded-2xl border bg-white p-4"><h2 className="text-2xl font-black">JAN・QRラベルの印刷</h2><p className="my-2 text-sm">種類を選び、必要なラベルにチェックしてまとめて印刷します。</p><div className="my-3 flex flex-wrap gap-2">{[["JAN", "商品JAN"], ["MAJOR", "大分類QR"], ["MINOR", "小分類QR"], ["LOCATION", "保管場所QR"]].map(([id, label]) => <button key={id} aria-pressed={kind === id} onClick={() => { setKind(id); setLimit(100); setSelected([]); setSearch(""); }} className={`rounded-xl border px-4 py-3 font-bold ${kind === id ? "bg-blue-700 text-white" : "bg-white"}`}>{label}</button>)}</div>
 <ImeInput type="search" aria-label="印刷するラベルを絞り込む" value={search} onChange={event => { setSearch(event.target.value); setLimit(100); }} placeholder="名前・JANで絞り込む（すぐ反映）" className="w-full rounded-xl border p-3"/>
 <p role="status" className="my-2">表示 {shown.length}件 ／ 選択 {selected.length}件{selected.some(id => !shown.some(row => row.id === id)) ? "（表示外の選択を含みます）" : ""}</p><div className="my-3 flex flex-wrap gap-2"><button disabled={!shown.length} className="rounded-xl border p-3" onClick={() => setSelected(current => [...new Set([...current, ...shown.map(row => row.id)])])}>検索結果を全選択</button><button className="rounded-xl border p-3" onClick={finish}>選択・絞り込みをクリア</button><button disabled={!selected.length} className="rounded-xl bg-blue-700 p-3 font-bold text-white disabled:opacity-40" onClick={() => setPrinting([...selected])}>選択したラベルをまとめて印刷</button></div>
 <div className="max-h-80 space-y-2 overflow-auto">{shown.slice(0, limit).map(row => <div key={row.id} className="flex items-center justify-between gap-3 rounded-xl border p-3"><label className="flex w-full min-w-0 cursor-pointer items-center gap-2"><ImeInput type="checkbox" checked={selected.includes(row.id)} onChange={event => setSelected(current => event.target.checked ? [...current, row.id] : current.filter(id => id !== row.id))}/><span className="break-words font-bold">{row.name}<span className="block break-all text-xs font-normal">{row.barcode}</span></span></label></div>)}</div>

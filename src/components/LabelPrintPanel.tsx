...
export default function LabelPrintPanel({ data, onRefresh }: {
 ...
 <div className="max-h-80 space-y-2 overflow-auto">{shown.slice(0, limit).map(row => <div key={row.id} className="flex items-center justify-between gap-3 rounded-xl border p-3"><label className="flex w-full min-w-0 cursor-pointer items-center gap-2"><input type="checkbox" checked={selected.includes(row.id)} onChange={event => setSelected(current => event.target.checked ? [...current, row.id] : current.filter(id => id !== row.id))}/><span className="break-words font-bold">{row.name}<span className="block break-all text-xs font-normal">{row.barcode}</span></span></label></div>)}</div>

...
export default function StocktakePhoto({item,large=false}:{item:PhotoItem;large?:boolean}){
  ...
  if(!src||failed===src)return <p className="mb-1 text-xs text-slate-400">{src?"写真を読み込めませんでした":"写真なし"}</p>;
  return <div className={"mb-3 flex w-full items-center justify-center rounded-xl bg-slate-50 "+(large?"h-32":"h-24")}>

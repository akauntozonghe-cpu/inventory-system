...
export default function StocktakeInputPanel({
  ...
  const difference = validQuantity
    ...
    <section className="rounded-3xl bg-white p-4 shadow-sm sm:p-6">
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          disabled={saving}
          onClick={onCancel}
          className="shrink-0 rounded-xl bg-slate-100 px-3 py-2 text-sm font-bold text-slate-700 disabled:opacity-50 sm:px-4"
        >
          戻る
        </button>
      </div>
      ...
      <dl className="mt-5 grid grid-cols-1 gap-x-5 gap-y-4 border-y border-slate-200 py-5 sm:grid-cols-2">
        ...
        <Detail label="システムバーコード" value={selected.item.systemBarcode} />
        ...
        <Detail
          label="分類"
          value={[selected.item.majorCategory, selected.item.minorCategory]
            .filter(Boolean)
            .join(" / ")}
        />
      ...
      <label className="mt-5 block">
        <span className="font-bold text-slate-900">棚卸数量</span>
        ...
        <input
          ...
          className="mt-2 w-full rounded-2xl border-2 border-blue-500 px-4 py-4 text-3xl font-black text-slate-950 outline-none focus:ring-4 focus:ring-blue-100 disabled:bg-slate-100"
        />
      </label>

      <div
        className={`mt-4 rounded-2xl px-4 py-4 text-center text-lg font-black ${
          difference === null
      ...
      <button
        type="button"
        disabled={disabled || saving || !validQuantity}
        onClick={onSave}
        className="mt-5 min-h-14 w-full rounded-2xl bg-blue-600 px-4 py-4 text-base font-black text-white transition hover:bg-blue-700 disabled:bg-slate-300 sm:text-lg"
      >
        {saving
          ? "保存中..."
          : continuous

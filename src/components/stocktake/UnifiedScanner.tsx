...
export default function UnifiedScanner({ onProduct, onCategory, onMinorCategory, onLocation, onClose, continuous = false, paused = false, children, title, notice, pauseMessage }: Props) {
  ...
  const detect = async (raw: string) => {
    ...
    try {
      ...
      if (scanned.type === "CLASSIFICATION" && scanned.name) {if(scanned.kind==="MINOR"){if(!onMinorCategory){setError("SCAN_MINOR_UNSUPPORTED：小分類QRは分類・保管場所の整理で使えます。");return;}await onMinorCategory(scanned.name,scanned.parentName??"");}else await onCategory(scanned.name); }

...
export default function InspectionTargets({ checkCode, runId, onChanged, onNavigate }: {
 ...
 {targets.map(target => <article key={target.id} className="rounded-xl border border-slate-300 bg-white p-4"><h4 className="break-words text-lg font-bold">{target.name}</h4>{target.owner && <p className="font-bold">担当者：{target.owner}</p>}<p className="my-2 break-all text-xs text-slate-500">{target.itemId && `管理No.：${target.itemId}　`}{target.inventoryId && `在庫No.：${target.inventoryId}　`}{target.sessionId && `棚卸No.：${target.sessionId}`}</p>{target.inventoryId && <p>Lot：{target.lot || "未設定"} ／ 保管場所：{target.location || "未設定"}</p>}<dl className="my-3 space-y-1"><div><dt className="font-bold">現在の状態</dt><dd>{target.current}</dd></div><div><dt className="font-bold">直した後の状態</dt><dd>{target.expected}</dd></div></dl>

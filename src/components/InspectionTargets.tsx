...
export default function InspectionTargets({ checkCode, runId, onChanged, onNavigate }: {
 ...
 {targets.map(target => <article key={target.id} className="rounded-xl border border-slate-300 bg-white p-4"><h4 className="break-words text-lg font-bold">{target.name}</h4>{target.owner && <p className="font-bold">担当者：{target.owner}</p>}{target.inventoryId && <p>Lot：{target.lot || "未設定"} ／ 保管場所：{target.location || "未設定"} ／ 期限：{target.expirationDate || "なし・未設定"} ／ 分類：{[target.majorCategory,target.minorCategory].filter(Boolean).join("・") || "未設定"} ／ 在庫No.：{target.inventoryId}</p>}<dl className="my-3 space-y-1"><div><dt className="font-bold">現在の状態</dt><dd>{target.current}</dd></div><div><dt className="font-bold">直した後の状態</dt><dd>{target.expected}</dd></div></dl>
 ...
 {selected?.id === target.id && <form className="mt-4 space-y-3 rounded-xl bg-slate-50 p-3" onSubmit={event => { event.preventDefault(); void save(); }}><p className="font-bold">変更対象：{target.name}{target.lot ? ` / Lot ${target.lot}` : ""}</p>{target.action === "SET_UNIT" && <label className="block font-bold">この在庫の数量単位<ImeInput required maxLength={30} list="inspection-unit-options" value={value} onChange={event => setValue(event.target.value)} className="mt-1 w-full rounded-xl border p-3" placeholder="例：個、箱"/><datalist id="inspection-unit-options"><option value="個"/><option value="箱"/><option value="本"/><option value="袋"/></datalist></label>}{target.action === "RESTORE_TARGET" && <label className="block font-bold">開始時の基準数量（記録で確認した値）<ImeInput required type="number" min={0} max={2147483647} step={1} value={value} onChange={event => setValue(event.target.value)} className="mt-1 w-full rounded-xl border p-3"/><span className="text-sm font-normal">数えた数量はそのまま残します。基準が分からない場合は保存せず、記録を確認してください。</span></label>}{target.action !== "ISSUE_SYSTEM_BARCODE" && <label className="block font-bold">変更理由<textarea required minLength={2} maxLength={500} value={reason} onChange={event => setReason(event.target.value)} className="mt-1 w-full rounded-xl border p-3"/></label>}<label className="flex gap-2"><ImeInput type="checkbox" checked={confirmed} onChange={event => setConfirmed(event.target.checked)}/>対象と変更内容を確認しました{target.action === "ISSUE_SYSTEM_BARCODE" && "（商品にJANが付いていないことを確認済み）"}</label><div className="flex flex-wrap gap-2"><button disabled={busy || !confirmed} type="submit" className="rounded-xl bg-blue-700 p-3 font-bold text-white disabled:opacity-50">この対象だけ保存して再確認</button><button disabled={busy} type="button" onClick={() => setSelected(null)} className="rounded-xl border p-3">変更せず戻る</button></div></form>}

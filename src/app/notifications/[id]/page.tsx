...
export default async function NotificationDetail({params}:{params:Promise<{id:string}>}){
 ...
 const session=effectiveSessionUser((await cookies()).get(AUTH_COOKIE)?.value,(await cookies()).get(ADMIN_ELEVATION_COOKIE)?.value);if(!session)redirect("/login?returnTo="+encodeURIComponent("/notifications/"+id));
 ...
 if(user && session.adminExpiresAt)user.role="ADMIN";

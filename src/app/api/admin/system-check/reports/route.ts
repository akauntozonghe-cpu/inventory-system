...
export async function GET(request:NextRequest) {
 ...
 try {const reports=await prisma.errorReport.findMany({where:{route,recoveryStatus:"ADMIN_REQUIRED"},orderBy:{occurredAt:"desc"},take:50,select:{id:true,code:true,message:true,recoveryAttempts:true}});return NextResponse.json({reports});}
...
export async function PATCH(request:NextRequest) {
 ...
 try {
  ...
  await prisma.$transaction(async tx=>{
   ...
   if(!run||run.mode!=="AUTO"||run.items.length===0||Date.now()-run.createdAt.getTime()>300000||run.items.some(item=>item.status==="FAIL"||item.status==="NOT_RUN"))throw new Error("RECHECK_REQUIRED");

...
const MODES = ["NORMAL", "INSPECTION", "MAINTENANCE"] as const;
...
export async function PATCH(request: NextRequest) {
  const auth = requireAdmin(request);
  if (auth.response) return auth.response;
  const body = await request.json().catch(() => null) as { mode?: unknown; message?: unknown } | null;
  ...
  const setting = await prisma.systemOperationSetting.upsert({
    where: { id: "system" },
    create: { id: "system", mode: body.mode, message, updatedByUserId: auth.user!.id },
    update: { mode: body.mode, message, updatedByUserId: auth.user!.id },
  });
  await prisma.adminActionLog.create({
    data: { adminUserId: auth.user!.id, action: "SYSTEM_OPERATION_MODE_UPDATE", route: "/admin/operation-mode", detail: { mode: body.mode, message } },
  });

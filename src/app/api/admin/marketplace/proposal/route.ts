...
export async function POST(request:NextRequest) {
  ...
  try {
    ...
    return reply({engine:"EVIDENCE",inventoryInstanceId:id,itemName:inventory.item.name,quantity,available,parcel,feeBps,feeNote:channelSetting?"共通設定の手数料率":feeBps!==null?"通常手数料の目安（キャンペーン・端数差は含みません）":"販売先の手数料を確認してください",methods,guide:SHIPPING_GUIDES[channel]??null,historyCount:history.length,missing:[...(!condition?["商品の状態を選ぶと、同じ商品の同じ状態の販売履歴を比較できます。"]:[]),...(Object.values(parcel).some(v=>v===null)?["発送する分をまとめて梱包した、大きさ・重さを確認してください。"]:[]),...(inventory.acquisitionCost===null?["原価が未登録のため、利益は確定できません。"]:[]),...(packaging===null?["梱包費は商品ごとに指定できます。専用BOX代は候補に含めています。"]:[]),...(methods.length?[]:["一致する送料の候補がありません。梱包サイズまたは販売先の公式案内を確認してください。"])],draft:{title:inventory.item.name,description:`${inventory.item.name}\n\n状態：${condition||"要確認"}\n付属品：要確認\n傷・動作：要確認\n\n写真と商品説明をご確認ください。`}});

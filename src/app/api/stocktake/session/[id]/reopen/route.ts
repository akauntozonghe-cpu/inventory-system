...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const session = await prisma.$transaction(async tx => {
      ...
      if (before.status !== "REVIEW") throw new Error("REOPEN_INVALID_STATUS");
      const changed = await tx.stocktakeSession.updateMany({ where: { id, status: before.status, updatedAt: before.updatedAt }, data: { status: "IN_PROGRESS", pausedAt: null } });
      if (changed.count !== 1) throw new Error("REOPEN_CHANGED");
      await tx.adminActionLog.create({ data: { adminUserId: auth.user!.id, action: "STOCKTAKE_REOPEN", route: `/stocktake/${id}`, detail: { sessionId: id, title: before.title, reason, previousStatus: before.status, newStatus: "IN_PROGRESS", recordsPreserved: true } } });
    ...
    const messages: Record<string, string> = {
      ...
      REOPEN_INVALID_STATUS: "入力終了・確認待ちの棚卸だけを再開できます。最新の状態を確認してください。",

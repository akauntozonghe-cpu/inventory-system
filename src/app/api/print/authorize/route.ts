...
export async function GET(request: NextRequest) { const auth = requireLogin(request); if (auth.response || !auth.user)
    return auth.response; const user = await prisma.appUser.findUnique({ where: { id: auth.user.id }, select: { role: true, isActive: true, featurePermissions: true } }); if (!user?.isActive || (auth.user.role !== "ADMIN" && user.role !== "ADMIN" && !user.featurePermissions.includes("LABEL_PRINT" as never)))

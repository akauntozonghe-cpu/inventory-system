      ...
      <div className="mx-auto max-w-7xl space-y-7">
        ...
        {liveFailed && <p role="status" className="rounded-xl bg-amber-50 p-3 text-amber-900">更新確認が遅れています。通信が戻ると再取得します。</p>}<div className={preparing?"hidden":"flex flex-wrap gap-3"}><ImeInput aria-label="出品を検索" value={listSearch} onChange={e=>{setListSearch(e.target.value);setPage(1);}} placeholder="商品名・出品名で検索" className="min-w-0 flex-1 rounded-xl border p-3"/><select aria-label="出品の状態" value={listStatus} onChange={e=>{setListStatus(e.target.value);setPage(1);}} className="rounded-xl border p-3"><option value="ACTIVE">対応中</option><option value="ALL">すべて</option>{Object.entries(statusLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div><section className="space-y-5">
          <div className={preparing?"mx-auto max-w-2xl rounded-3xl bg-white p-5 shadow-sm sm:p-7":"hidden"}><h2 className="text-xl font-black">1. 出品する在庫を選ぶ</h2><button onClick={()=>setScannerOpen(true)} className="mt-3 w-full rounded-xl bg-slate-800 p-3 font-bold text-white">JAN・QRを読み取る</button><ImeInput aria-label="出品する在庫を検索" value={inventoryQuery} onChange={e=>{setInventoryQuery(e.target.value);setInventoryId("");}} onKeyDown={e=>{if(e.key==="Enter"&&!e.nativeEvent.isComposing){e.preventDefault();void searchCode();}}} placeholder="商品名・JANで検索／QRを入力してEnter" className="mt-3 w-full rounded-xl border p-3"/><ActiveFilters filters={[{label:"大分類",value:categoryFilter,remove:()=>{setCategoryFilter("");setInventoryId("");}},{label:"保管場所",value:locationFilter,remove:()=>{setLocationFilter("");setInventoryId("");}},{label:"検索",value:inventoryQuery,remove:()=>{setInventoryQuery("");setInventoryId("");}}]} onClear={()=>{setCategoryFilter("");setLocationFilter("");setInventoryQuery("");setInventoryId("");}}/><p className="my-2 text-sm">大分類QRはその分類、保管場所QRは棚・部屋などの場所から出品可能な在庫を探すために使います。両方読むと条件を組み合わせます。読み取るだけでは在庫を移動しません。</p><div className="mt-4 space-y-3">
            ...
            <label className="block text-sm font-bold">何点出品しますか<ImeInput type="number" min="1" max={selectedInventory?.storageAvailable??1} value={quantity} onChange={event=>setQuantity(event.target.value)} aria-label="出品数" className="mt-1 w-full rounded-xl border p-3"/></label>
            ...
            <ImeInput aria-label="出品タイトル" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="出品タイトル" className="w-full rounded-xl border p-3" />
            ...
            <div className="grid grid-cols-2 gap-2"><ImeInput type="number" value={price} onChange={(event) => setPrice(event.target.value)} aria-label="販売価格" placeholder="1点の販売価格（円）" className="rounded-xl border p-3" /></div>
            <div className="grid grid-cols-2 gap-2"><ImeInput value={shippingMethod} onChange={(event) => setShippingMethod(event.target.value)} placeholder="配送方法" className="rounded-xl border p-3" /><ImeInput type="number" value={shippingCost} onChange={(event) => setShippingCost(event.target.value)} placeholder="予定送料" className="rounded-xl border p-3" /></div>
            <label className="block text-sm font-bold">今回の梱包費（円）<ImeInput aria-label="今回の梱包費" type="number" min="0" value={packagingCost} onChange={e=>setPackagingCost(e.target.value)} placeholder="袋・箱・緩衝材の合計" className="mt-1 w-full rounded-xl border p-3"/></label>

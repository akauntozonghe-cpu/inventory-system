  ...
  return (
    <main className="min-h-screen bg-violet-50 p-4 text-slate-950 sm:p-8">
      ...
      <div className="mx-auto max-w-7xl space-y-7">
        <header className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-black tracking-[0.2em] text-violet-700">PERSONAL FLEA MARKET</p><h1 className="mt-1 text-3xl font-black">個人フリマ統合管理</h1><p className="mt-2 text-slate-600">出品原稿、併売、在庫、梱包、発送、利益まで一か所で管理します。</p></div><div className="flex gap-2"><a href="/api/admin/marketplace/listings?format=csv" className="rounded-xl bg-emerald-600 px-4 py-3 font-black text-white">取引CSV</a><Link href="/" className="rounded-xl bg-slate-800 px-4 py-3 font-black text-white">ホーム</Link></div></header><SectionNavigation label="フリマ" current="/marketplace" links={marketplaceLinks} />
        ...
        {liveFailed && <p role="status" className="rounded-xl bg-amber-50 p-3 text-amber-900">更新確認が遅れています。通信が戻ると再取得します。</p>}<div className="flex flex-wrap gap-3"><input aria-label="出品を検索" value={listSearch} onChange={e=>{setListSearch(e.target.value);setPage(1);}} placeholder="商品名・出品名で検索" className="min-w-0 flex-1 rounded-xl border p-3"/><select aria-label="出品の状態" value={listStatus} onChange={e=>{setListStatus(e.target.value);setPage(1);}} className="rounded-xl border p-3"><option value="ACTIVE">対応中</option><option value="ALL">すべて</option>{Object.entries(statusLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div><section className="grid gap-6 lg:grid-cols-[420px_1fr]">
          <div className="h-fit rounded-3xl bg-white p-5 shadow-sm lg:sticky lg:top-5"><h2 className="text-xl font-black">在庫から出品準備</h2><div className="mt-4 space-y-3">
            ...
            <button type="button" disabled={!!working || !inventoryId || !price} onClick={() => void createDraft()} className="w-full rounded-xl bg-violet-700 px-4 py-3 font-black text-white disabled:opacity-40">出品準備へ追加</button>
          </div></div>
          ...
          <div className="space-y-4"><Pagination page={currentPage} totalPages={pages} start={(currentPage-1)*30} end={Math.min(currentPage*30, visibleListings.length)} total={visibleListings.length} onPageChange={setPage} />{visibleListings.length === 0 ? <div className="rounded-3xl bg-white p-8 text-center text-slate-500">この条件に該当する出品はありません。</div> : pageRows.map((listing) => {
            ...
            return <article key={listing.id} className="rounded-3xl bg-white p-5 shadow-sm"><div className="flex flex-wrap justify-between gap-3"><div><p className="text-xs font-black text-violet-700">{channelLabels[listing.channel] ?? listing.channel} ・ {statusLabels[listing.status] ?? listing.status}</p><h2 className="mt-1 text-xl font-black">{listing.title}</h2><p className="mt-1 text-sm text-slate-500">在庫：{listing.inventoryInstance.item.name}</p></div><div className="text-right"><p className="text-2xl font-black">{listing.price.toLocaleString("ja-JP")}円</p><p className={`text-sm font-bold ${profit >= 0 ? "text-emerald-700" : "text-red-600"}`}>概算利益 {profit.toLocaleString("ja-JP")}円</p></div></div>
              <div className="mt-4 flex flex-wrap gap-2"><button onClick={() => void copyDraft(listing)} className="rounded-xl bg-slate-100 px-3 py-2 text-sm font-black">出品文をコピー</button>{listing.listingUrl && <a href={listing.listingUrl} target="_blank" rel="noreferrer" className="rounded-xl bg-blue-50 px-3 py-2 text-sm font-black text-blue-700">出品ページ</a>}{listing.status === "DRAFT" && <button disabled={!!working} onClick={() => void update(listing.id, { status: "READY" })} className="rounded-xl bg-amber-500 px-3 py-2 text-sm font-black text-white">原稿完成</button>}{listing.status === "READY" && <button disabled={!!working} onClick={() => void update(listing.id, { status: "LISTED" })} className="rounded-xl bg-violet-700 px-3 py-2 text-sm font-black text-white">出品済みにする</button>}{listing.status === "LISTED" && <button disabled={!!working} onClick={() => void update(listing.id, { status: "SOLD", soldQuantity: listing.listedQuantity })} className="rounded-xl bg-emerald-600 px-3 py-2 text-sm font-black text-white">売れた・在庫反映</button>}{!["SOLD", "CANCELLED"].includes(listing.status) && <button disabled={!!working} onClick={() => void update(listing.id, { status: "CANCELLED" })} className="rounded-xl bg-slate-700 px-3 py-2 text-sm font-black text-white">取下げ</button>}</div>
              {listing.status === "SOLD" && <div className="mt-4 rounded-2xl bg-orange-50 p-4"><p className="font-black text-orange-900">発送：{shippingLabels[listing.shippingStatus] ?? listing.shippingStatus}</p><div className="mt-3 flex flex-wrap gap-2">{[["PACKING", "梱包中"], ["READY_TO_SHIP", "発送待ち"], ["SHIPPED", "発送済み"], ["DELIVERED", "配達済み"], ["SETTLED", "取引完了"]].map(([value, label]) => <button key={value} disabled={!!working} onClick={() => void update(listing.id, { action: "UPDATE_SHIPPING", shippingStatus: value })} className="rounded-lg bg-white px-3 py-2 text-xs font-black shadow-sm">{label}</button>)}</div></div>}
            </article>;
          })}</div>
        </section>
      </div>

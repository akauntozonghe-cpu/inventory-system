...
export default function ExpiryPage() {
  ...
  const entries = useMemo(() => data.entries.filter((entry) => {
    ...
    const matchesFilter = filter === "ALL" || (filter === "ACTION" && ["EXPIRED", "TODAY", "CRITICAL", "WARNING", "INVALID"].includes(entry.assessment.level) && entry.expirationManagementStatus !== "RESOLVED") || (filter === "NO_EXPIRY" && entry.assessment.level === "NONE" && entry.expirationManagementStatus === "NO_EXPIRY") || filter === entry.assessment.level || filter === entry.expirationManagementStatus;

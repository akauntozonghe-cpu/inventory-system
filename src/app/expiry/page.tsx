...
type Entry = {
  id: string; majorCategory: string | null; minorCategory: string | null; expirationDate: string | null; expirationAlertDays: number; expirationManagementStatus: string;
...
export default function ExpiryPage() {
  ...
  const entries = useMemo(() => data.entries.filter((entry) => {
    ...
    const matchesIssue = dataIssue === "NONE" || (dataIssue === "MAJOR" && !entry.majorCategory) || (dataIssue === "MINOR" && !entry.minorCategory) || (dataIssue === "LOCATION" && !entry.storageLocation);
    const itemId = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("itemId") : null;
    const inventoryId = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("inventoryId") : null;
    return (!inventoryId || entry.id === inventoryId) && (!itemId || entry.item.id === itemId) && matchesSearch && matchesFilter && matchesIssue;

      ...
      <div className="mx-auto max-w-7xl space-y-6 p-5 sm:p-8">
        ...
        {isReview && (
          <section className="rounded-3xl border border-indigo-200 bg-indigo-50 p-6">
            ...
            {data.summary.recordedCount === 0 && <p role="status" className="mt-4 rounded-xl bg-amber-100 p-4 font-bold text-amber-950">保存済みの棚卸数量がないため、このまま正式確定はできません。{data.permissions.isAdmin ? "上の「途中から再開」から入力を再開してください。" : "管理者に再開を依頼してください。"}</p>}
        ...
        <section className="rounded-3xl bg-white p-5 shadow-sm sm:p-7">
          ...
          {focusedInventory&&<p className="mb-3 rounded-xl bg-blue-50 p-3 font-bold">点検で指定された在庫の記録を表示中<button className="ml-3 underline" onClick={()=>setFocusedInventory("")}>この棚卸の全記録を表示</button><Link href="/admin/recovery" className="ml-3 underline">点検へ戻る</Link></p>}{(data.unlinkedRecords??[]).filter(row=>!focusedInventory||row.inventoryInstanceId===focusedInventory).map(row=><article key={row.id} className="my-3 rounded-xl border border-amber-300 bg-amber-50 p-4"><h3 className="font-bold">対象登録の確認が必要：{row.name}</h3><p>Lot：{row.lotNo||"未設定"} ／ 保管場所：{row.location||"未設定"}</p><p>保存済みの数量：{row.countedQuantity} ／ メモ：{row.memo||"なし"}</p><p>この担当者の入力は保存されていますが、この棚卸の対象登録がありません。数量を消さずに管理者が点検画面で対応してください。</p>{data.permissions.isAdmin&&<Link href="/admin/recovery" className="underline">点検でこの記録の対応を確認</Link>}</article>)}<SearchBar value={resultSearch} onChange={setResultSearch} placeholder="商品・Lot・保管場所・メモで検索" className="mt-5" />
          <label className="mt-3 flex items-center gap-3 rounded-xl bg-slate-100 p-3 font-bold"><ImeInput type="checkbox" checked={differenceOnly} onChange={event => setDifferenceOnly(event.target.checked)} className="h-5 w-5" />差異ありのみ表示</label>
          <label className="mt-5 flex items-center gap-3 rounded-xl bg-slate-100 p-3 font-bold">
            <ImeInput type="checkbox" checked={memoOnly} onChange={(event) => setMemoOnly(event.target.checked)} className="h-5 w-5" />

...
export default function ClassificationsPage() {
  ...
  return <main className="min-h-screen bg-slate-100 p-4 text-slate-950 sm:p-8">
    ...
    <div className="mx-auto max-w-7xl"><header className="flex flex-wrap justify-between gap-4"><div><p className="text-sm font-black tracking-widest text-indigo-700">CLASSIFICATION CONTROL</p><h1 className="mt-1 text-3xl font-black">分類・保管場所の整理</h1><p className="mt-2 text-slate-600">追加、名称変更、統合、移動、未使用分類の削除を、商品・在庫・棚卸範囲へ一括反映します。</p></div><div className="flex h-fit flex-wrap gap-2"><button onClick={()=>setScannerOpen(true)} className="rounded-xl bg-indigo-600 px-4 py-3 font-black text-white">JAN・QRで検索</button><Link href="/admin" className="rounded-xl bg-slate-800 px-4 py-3 font-black text-white">システム管理へ</Link></div></header>
      <section className="mt-5 rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-sm font-bold text-indigo-950">JAN・商品QRは商品の選択、大分類QRは分類の絞り込みに使います。保管場所は名前で選ぶだけでも使えます。QRを棚に貼ると場所の選択を省けます。小分類は大分類の中の細分け（例：食品→飲料）に使い、コードの印刷は不要です。</section>

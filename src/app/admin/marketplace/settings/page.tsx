    ...
    <div className="mx-auto max-w-5xl"><header className="flex flex-wrap items-center justify-between gap-3"><h1 className="text-3xl font-bold">フリマの共通設定</h1><Link href="/marketplace" className="rounded-xl bg-slate-800 px-4 py-3 font-bold text-white">出品管理へ</Link></header>
      ...
      <fieldset disabled={!ready || busy}>
        <section className="mt-6 rounded-2xl bg-white p-5"><h2 className="text-xl font-bold">発送地と発送の目安</h2>
          ...
          <label className="mt-5 block text-sm font-bold">提案する価格の目標利益率（%）<ImeInput aria-label="目標利益率" type="number" min="0" max="90" step="0.1" value={target} onChange={e => changeGeneral(() => setTarget(e.target.value))} className={field}/></label>
        ...
        <section className="mt-6 rounded-2xl bg-white p-5"><h2 className="text-xl font-bold">販売先の手数料</h2><p className="mt-2 text-sm text-slate-600">手取り額と利益の計算に使います。割引や実績による料率は販売先のアカウントで確認し、適用する割合を保存してください。</p>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">{Object.entries(channelNames).map(([channel, name]) => <div key={channel} className="rounded-xl border p-4"><label className="text-sm font-bold">{name}（%）<ImeInput aria-label={`${name}の手数料`} type="number" min="0" max="100" step="0.1" placeholder="手数料を確認して入力" value={fees[channel] ?? ""} onChange={e => { dirty.current.add(channel); setFees(current => ({ ...current, [channel]: e.target.value })); }} className={field}/></label><button disabled={fees[channel] === undefined || fees[channel] === "" || !Number.isFinite(Number(fees[channel])) || Number(fees[channel]) < 0 || Number(fees[channel]) > 100} onClick={() => void post({ action: "SAVE_CHANNEL_FEE", channel, feeRateBps: Math.round(Number(fees[channel]) * 100) }, channel)} className={`${button} mt-3`}>手数料を保存</button></div>)}</div>
        </section>
        <details className="mt-6 rounded-2xl bg-white p-5"><summary className="cursor-pointer text-lg font-bold">独自の送料を登録する</summary><p className="mt-2 text-sm text-slate-600">対象の販売先・発送地に合う登録料金を優先します。送料が宛先で変わる場合は、この料金で発送できることを確認して使ってください。</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">{([['carrier','配送会社'],['methodName','発送方法'],['fee','送料（円）'],['maxWeightGrams','最大重量（g）'],['maxTotalDimensionsCm','3辺合計（cm）']] as const).map(([key,label]) => <label key={key} className="text-sm font-bold">{label}<ImeInput value={shipping[key]} type={key === 'carrier' || key === 'methodName' ? 'text' : 'number'} min="0" step="1" onChange={e => changeShipping({ [key]: e.target.value })} className={field}/></label>)}
          ...
          </div><div className="mt-3 flex flex-wrap gap-4">{([['anonymous','匿名'],['tracking','追跡'],['compensation','補償']] as const).map(([key,label]) => <label key={key}><ImeInput type="checkbox" checked={shipping[key]} onChange={e => changeShipping({ [key]: e.target.checked })}/> {label}</label>)}</div>

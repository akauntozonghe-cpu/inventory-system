...
export async function inspectionTargets(code: string, sessionId: string | null, cursor: string | null) {
    ...
    const condition = code === "CHECK_PRODUCT_LINKS" ? Prisma.sql `(v.manufacturer IS DISTINCT FROM i.manufacturer)` :
    ...
    const ids = await prisma.$queryRaw<Array<{
        id: string;
    }>>(Prisma.sql `SELECT v.id FROM "InventoryInstance" v JOIN "Item" i ON i.id=v."itemId" WHERE i."isArchived"=false AND COALESCE(v."inspectionExcluded",i."inspectionExcluded")=false AND v.status<>'廃止' AND ${condition} ${cursor ? Prisma.sql `AND v.id>${cursor}` : Prisma.empty} ORDER BY v.id LIMIT 51`);
    ...
    return { targets: rows.filter(row => code !== "CHECK_INVALID_UNITS" || unitValidationMessage(row.unit) || unitValidationMessage(row.item.defaultUnit)).map(row => {
            const differences = (["manufacturer"] as const).filter(key => row[key] !== row.item[key]).map(key => ({ field: ({ majorCategory: "大分類", minorCategory: "小分類", manufacturer: "メーカー" })[key], before: row[key], after: row.item[key] }));
            const base = { id: row.id, itemId: row.itemId, inventoryId: row.id, name: row.item.name, lot: row.lotNo, location: row.storageLocation?.name, expirationDate: row.expirationDate, majorCategory: row.majorCategory ?? row.item.majorCategory, minorCategory: row.minorCategory ?? row.item.minorCategory, updatedAt: row.updatedAt.toISOString(), itemUpdatedAt: row.item.updatedAt.toISOString(), href: `/items/${row.itemId}?inventoryId=${row.id}#inventory-${row.id}` };
            if (code === "CHECK_PRODUCT_LINKS")
                return { ...base, differences, current: "在庫と商品のメーカーが一致していません", expected: "下記の変更後の値（商品情報）", instruction: "この在庫明細だけを、表示した商品情報へ揃えます。分類・期限・数量・Lot・他ユーザーの棚卸入力は変えません。", action: "SYNC_PRODUCT_METADATA" } satisfies InspectionTarget;
